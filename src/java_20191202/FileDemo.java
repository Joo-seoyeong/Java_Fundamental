...
public class FileDemo {
	public static void main(String[] args){
		...
		boolean isSuccess = f1.mkdirs(); // 디렉토리가 잇으면 false, 새로 만들면 true
		System.out.println(isSuccess);
		
		File f2 = new File(f1, "jdk-11.0.3_windows-x64_bin.exe"); 
		// ("c:\\dev\\io\\2019\\12\\jdk-11.0.3_windows-x64_bin.exe")
		long fileSize = f2.length()/1024;
		System.out.println(fileSize);
		long lastModified = f2.lastModified();
		
		Calendar c = Calendar.getInstance(); 
		// calendar는 인터페이스나 추상 클래스(원래 객체 생성 못함)라서 자식의 도움을 받아 getInstance를 통해 객체 생성 
		...
		// a.txt => 123123123123123.txt로 바꾸기
		String extension = 
